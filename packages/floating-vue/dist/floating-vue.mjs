  ...
  methods: {
    ...
    async $_computePosition() {
      ...
      if (t ? e.middleware.push(Ue({
        ...
        name: "arrowOverflow",
        fn: ({ placement: i, rects: s, middlewareData: p }) => {
          let a;
          const { centerOffset: n } = p.arrow;
          return i.startsWith("top") || i.startsWith("bottom") ? a = Math.abs(n) > s.reference.width / 2 : a = Math.abs(n) > s.reference.height / 2, {
            data: {
              overflow: a
            }
          };
        ...
        e.middleware.push({
          name: "autoSize",
          fn: ({ rects: s, placement: p, middlewareData: a }) => {
            var h;
            if ((h = a.autoSize) != null && h.skip)
              return {};
            let n, d;
            return p.startsWith("top") || p.startsWith("bottom") ? n = s.reference.width : d = s.reference.height, this.$_innerNode.style[i === "min" ? "minWidth" : i === "max" ? "maxWidth" : "width"] = n != null ? `${n}px` : null, this.$_innerNode.style[i === "min" ? "minHeight" : i === "max" ? "maxHeight" : "height"] = d != null ? `${d}px` : null, {
              data: {
                skip: !0
              },
              reset: {
    ...
    async $_applyShowEffect() {
      ...
      if (this.computeTransformOrigin) {
        const t = this.$_referenceNode.getBoundingClientRect(), o = this.$_popperNode.querySelector(".v-popper__wrapper"), i = o.parentNode.getBoundingClientRect(), s = t.x + t.width / 2 - (i.left + o.offsetLeft), p = t.y + t.height / 2 - (i.top + o.offsetTop);
        this.result.transformOrigin = `${s}px ${p}px`;
      }
      ...
      if (e) {
        ...
        for (let o = 0; o < l.length; o++)
          t = l[o], t.showGroup !== e && (t.hide(), t.$emit("close-group"));
      }
    ...
    async $_applyHide(e = !1) {
      ...
      if (clearTimeout(this.$_scheduleTimer), !this.isShown)
        return;
      this.skipTransition = e, de(l, this), l.length === 0 && document.body.classList.remove("v-popper--some-open");
    ...
    $_registerTriggerListeners(e, t, o, i, s) {
      let p = o;
      i != null && (p = typeof i == "function" ? i(p) : i), p.forEach((a) => {
        const n = t[a];
        n && this.$_registerEventListeners(e, n, s);
      });
    },
    $_removeEventListeners(e) {
      const t = [];
      this.$_events.forEach((o) => {
        const { targetNodes: i, eventType: s, handler: p } = o;
        !e || e === s ? i.forEach((a) => a.removeEventListener(s, p)) : t.push(o);
      }), this.$_events = t;
    },
    $_refreshListeners() {
    ...
    $_isAimingPopper() {
      ...
      if (y >= e.left && y <= e.right && _ >= e.top && _ <= e.bottom) {
        const t = this.$_popperNode.getBoundingClientRect(), o = y - c, i = _ - m, p = t.left + t.width / 2 - c + (t.top + t.height / 2) - m + t.width + t.height, a = c + o * p, n = m + i * p;
        return C(c, m, a, n, t.left, t.top, t.left, t.bottom) || // Left edge
        C(c, m, a, n, t.left, t.top, t.right, t.top) || // Top edge
        C(c, m, a, n, t.right, t.top, t.right, t.bottom) || // Right edge
        C(c, m, a, n, t.left, t.bottom, t.right, t.bottom);
      }
      return !1;
    }
  },
...
function Pe(e, t) {
  ...
  for (let i = l.length - 1; i >= 0; i--) {
    const s = l[i];
    try {
      const p = Array.from(s.shownChildren).some((n) => {
        const d = l.find((h) => h.randomId === n);
        return d && d.popperNode().contains(e.target);
      }), a = s.containsGlobalTarget = s.mouseDownContains || s.popperNode().contains(e.target) || p;
      s.pendingHide = !1, requestAnimationFrame(() => {
        if (s.pendingHide = !1, !o[s.randomId] && ce(s, a, e)) {
          if (s.$_handleGlobalClose(e, t), !e.closeAllPopover && e.closePopover && a) {
            let d = s.parentPopper;
            for (; d; )
              o[d.randomId] = !0, d = d.parentPopper;
            return;
          }
          let n = s.parentPopper;
          for (; n && ce(n, n.containsGlobalTarget, e); ) {
...
function C(e, t, o, i, s, p, a, n) {
  const d = ((a - s) * (t - p) - (n - p) * (e - s)) / ((n - p) * (o - e) - (a - s) * (i - t)), h = ((o - e) * (t - p) - (i - t) * (e - s)) / ((n - p) * (o - e) - (a - s) * (i - t));
...
Ie();
const at = /* @__PURE__ */ rt((e, t, o, i, s, p) => (f(), M("div", pt)));
E.render = at;
...
function mt(e, t, o, i, s, p) {
  const a = P("ResizeObserver");
  return f(), T("div", {
    id: e.popperId,
    ref: "popover",
    ...
    w("div", {
      ...
      w("div", ht, [
        e.mounted ? (f(), T(Ve, { key: 0 }, [
          ...
          e.handleResize ? (f(), M(a, {
            key: 0,
            onNotify: t[1] || (t[1] = (n) => e.$emit("resize", n))
          })) : se("", !0)
        ], 64)) : se("", !0)
      ], 512),
      w("div", {
...
function wt(e, t, o, i, s, p) {
  const a = P("PopperContent"), n = P("Popper");
  return f(), M(n, $e({ ref: "popper" }, e.$props, {
    theme: e.finalTheme,
...
function _t(e, t, o, i, s, p) {
  const a = P("PopperContent"), n = P("Popper");
  return f(), M(n, $e({ ref: "popper" }, e.$attrs, {
    theme: e.theme,
...
function bt(e, t, o) {
  St();
  const i = U(Ne(e, t, o)), s = U(!1), p = {
    id: Pt++,
    options: i,
    shown: s
  };
  return b.value.push(p), e.classList && e.classList.add(Ae), e.$_popper = {
    options: i,
    item: p,
    show() {
      s.value = !0;
...
}, Ht = u, Ot = oe, Mt = oe, Bt = ie, Et = ie, kt = Se, Lt = be, Dt = Q, It = ee, Ft = te, Rt = k, jt = Z, Vt = Ce, Wt = ze;
function Ct(e, t = {}) {
  e.$_vTooltipInstalled || (e.$_vTooltipInstalled = !0, ye(u, t), e.directive("tooltip", oe), e.directive("close-popper", ie), e.component("VTooltip", Ce), e.component("VDropdown", Se), e.component("VMenu", be));
}
const Gt = {
